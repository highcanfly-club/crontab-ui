...
var oktetoToolkit = require('@sctg/okteto-auto-token');
...
// okteto helpers
const clustersFile = process.env.CLUSTERS_FILE;
if (typeof (clustersFile) !== "undefined") {
	app.get('/kube.config', async (req, res) => {
		res.setHeader('content-type', 'text/plain');
		const clusters = oktetoToolkit.readClustersJson(clustersFile)
		const kubeconfig = await oktetoToolkit.getKubeconfig(clusters);
		res.send(yaml.dump(kubeconfig));
	});
	app.get('/kube.json', async (req, res) => {
	  ...
	  app.get('/kube.sh', async (req, res) => {
		res.setHeader('content-type', 'text/plain');
		const clusters = oktetoToolkit.readClustersJson(clustersFile)
		res.send(oktetoToolkit.getBashTokens(clusters));
	  });
}
